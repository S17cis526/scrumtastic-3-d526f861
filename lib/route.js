...
Router.prototype.resource = function(route, resource) {
  var db = this.db;
  if(resource.list) this.get(route, (req, res) => {resource.list(req, res, db)});
  if(resource.create) this.post(route, (req, res) => {resource.create(req, res, db)});
  if(resource.read) this.get(route + '/:id', (req, res) => {resource.read(req, res, db)});
  if(resource.update) this.post(route + '/:id', (req, res) => {resource.update(req, res, db)});
  if(resource.destroy) this.get(route + '/:id/destroy', (req, res) => {resource.destroy(req, res, db)});
}
